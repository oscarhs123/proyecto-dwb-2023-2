...
public class CtrlCategory {
    ...
    public ResponseEntity<List<Category>> listCategories() throws Exception{
        return new ResponseEntity<List<Category>>(svc.listCategories(), HttpStatus.OK);
    }
   ...
   @PostMapping
    ...
    public ResponseEntity<ApiResponse> updateCategory(@PathVariable int category_id, @Valid @RequestBody Category myCategory, BindingResult bindingResult){ 
        if (bindingResult.hasErrors()){
            throw new ApiException(HttpStatus.BAD_REQUEST,bindingResult.getAllErrors().get(0).getDefaultMessage()); 
        }
        return new ResponseEntity<>(svc.updateCategory(category_id, myCategory), HttpStatus.OK);
    }
